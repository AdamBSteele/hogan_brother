...
# Initialize logging
logfile = os.environ.get('KOTHPATH') + 'log.txt'
...
keyfile = os.environ.get('KOTHPATH') + 'keyfile.txt'
Odict = {}
...
for status in reversed(sourceStatuses[0:10]):
    ...
    # Grab creation time from Twitter & remove UTC stuff that datetime doesn't
    # like
    strTime = ' '.join(
    ...
    if tweet_age.seconds < 180 and dtime.day == startTime.day:
        ...
        logging.info("age=%3ss POST: \'%s\'" %
                     (str(tweet_age.seconds), newTweet))
        try:
